      ...
      <div className="space-y-4">
        {posts.map(post => (
          <Link key={post.id} href={`/post/${post.id}`} className="block">
            <div className="bg-neutral-800 hover:bg-neutral-700 transition rounded-lg p-5 shadow-md border border-neutral-700">
              <h2 className="text-xl font-semibold text-gray-400 hover:underline truncate">{post.title}</h2>
              <p className="text-sm text-neutral-400 mt-1">By {post.author || 'Anonymous'}</p>
